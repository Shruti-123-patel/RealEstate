...
<body>
	...
	<div class="container mt-3">
		<div class="row">
			<div class="col-md-12">
				<h1 class="text-center mb-3">Agents</h1>
				...
				<table class="table" style="border: 3">
					<thead class="thead-dark">
						<tr>
							...
							<th scope="col"><a href="addAgent" class="nav-link">+ Add Agent </a></th>
						</tr>
					</thead>
					<tbody>
						<c:forEach var="p" items="${agent}">
							<tr>
								<td>${p.getName()}</td>
								<td>${p.getPhoneNo()}</td>
								<td>${p.getEmail()}</td>
								<td>${p.getCity()}</td>
								<td><a href="updateAgent?id=${p.getId() }">Update</a></td>
								<td><a href="removeAgent?id=${p.getId() }">Remove</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
